...
namespace Laura {
	...
	OpenGLTexture::OpenGLTexture(const std::string& filepath, int bindingPoint)
		: m_Filepath(filepath), m_BindingPoint(bindingPoint), m_ID(0), m_Width(0), m_Height(0), m_Channels(0) {
	...
	OpenGLTexture::OpenGLTexture(int width, int height, int channels, int bindingPoint)
		: m_Width(width), m_Height(height), m_Channels(channels), m_BindingPoint(bindingPoint), m_ID(0), m_Filepath("") {
		...
		GLCall(glBindImageTexture(m_BindingPoint, m_ID, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA32F));
	}
