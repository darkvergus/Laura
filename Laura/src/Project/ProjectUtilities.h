...
namespace Laura 
{
	...
	// search for all files with the given extension in the folder
	...
	/// append an extension to the path (eg. asset/cube.fbx -> asset/cube.fbx.lrmeta)
	inline std::filesystem::path AppendExtension(const std::filesystem::path& path, const std::string& extension) {
		return path + extension;
	}

	/// strip the last extension from a file (eg. asset/cube.fbx.lrmeta -> asset/cube.fbx)
